...
// Hexagon/code icon SVG path (24x24 viewBox)
const CODE_ICON = Setting;
...
export default function RawDp({ dp }: { dp: DpEntry }) {
  ...
  const actions = useActions() as unknown as Record<string, { set: (v: any) => Promise<boolean> }>;
    ...
    <View className={styles.rawCard}>
      <View className={styles.rawHeader}>
        <View className={styles.rawIcon}>
          <Icon name={CODE_ICON} size="34rpx" color="var(--app-M4, #ff7a45)" />
        </View>
